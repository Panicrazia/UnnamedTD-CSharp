...
public class GameScene : Node2D
{
    ...
    public BuildingBase currentSelection = null; //should probably make this a node2d and have it determine if things are selectable with an ISelectable interface
    ...
    public object[] selectionInfo = new object[]{};
    public WeakRef selectionNode = null;
    ...
    public override void _Ready()
    {
        ...
        {
            i.Connect("pressed", this, "InitiateBuildMode", new Array() { i.Name });
        }
    }
    ...
    public override void _Process(float delta)
    {
        if (buildingMode)
        {
            TextureRect preview = (TextureRect)mapNode.GetNode("BuildingPreview");
            ...
            switch (buildingCategory)
            {
                case "Building":
                    ...
                    preview.RectGlobalPosition = ((buildLocation - new Vector2(1, 1)) * CELL_SIZE) + mapNode.Position;
                    break;
                case "Accessory":
                    ...
                    if (snap) //if it should snap then snap to said location
                    {
                        ...
                        preview.RectGlobalPosition = relevantBuilding.Position - new Vector2(CELL_SIZE, CELL_SIZE);
    ...
    public void CancelBuildMode()
    {
        buildingMode = false;
        buildValid = false;
        mapNode.GetNode<TextureRect>("BuildingPreview").Hide();
    }
    ...
    public bool VerifyTilePositionsAsBuildable(int size, Vector2 pos, bool checkCollisions)
    {
        if (checkCollisions)
        {
            ...
            TileMap buildings = mapNode.GetNode<TileMap>("Buildings");
            ...
            foreach (Vector2 value in positions)
            {
                ...
                if ((check == 0) || (check == 1) || (buildings.GetCellv(value) != -1))
                {
                    return false;
                }
            }
    ...
    // are these two accessory methods possibly going to be useless? yes, but I have them just incase
